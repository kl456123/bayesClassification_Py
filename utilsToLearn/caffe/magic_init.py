...
#output_names usually is [top_names]
#NIT = batch_size
...
def gatherInputData(net, layer_id, bottom_data, top_name, fast=False, max_data=None):
    ...
    #activation data blob in the previous layer That is to say all bottom data
	...
	for i in range(OS):
        ...
        #i means for top data
        #here data [NIT*N*H*W,K]
        #represent in the position (h,w) every output of node in each batch
...
def initializeLayer(net, layer_id, bottom_data, top_name, bias=0, type='elwise', max_data=None):
	...
	if type == 'elwise':
		d = l.blobs[0].data
        #weights are initiated by random
		d[...] = np.random.normal(0, 1, d.shape)
	else: # Use the input data
		# Are there any groups?
        #note that the groups is for strict the connections between input and output
        #default G is 1
		G = 1
        #bottom names in the layer indexed by layer_id and it is a array contains all bottom blobs' names
		bottom_names = net.bottom_names[net._layer_names[layer_id]]
        #just one bottom data
        #note that blob is (N,K,H,W) K refers to channel 
        #weights is (Out,In,H,W)
        #so N1=K and N2=weights' number of Innode
